    ...
    while queue :
      ...
      for neighbour in graph.neighbors(node):
        if node2distances[node] < max_depth:
          if neighbour not in node2distances and neighbour != root:
             node2distances[neighbour] = node2distances[node] +1
             node2num_paths[neighbour] = node2num_paths[neighbour] +1
             node2parents[node] = [neighbour]
             queue.append(neighbour)


          if node2distances[neighbour] == node2distances[node] + 1 and neighbour != root:
             node2num_paths[neighbour] = node2num_paths[neighbour] +1
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for node in node2distances:
       if dict[node[0]] != root :
        parent = node2parents[node]
        if len(parent) == 1:
          dict[parent] = dict[parent] + dict[node]
          r = tuple(sorted(node, parent))
          result [r] = dict[node]
        else:
          for p in parent:
            dict[p] += dict[node] / len(parent)
            r = tuple(sorted(node, p))
            result [r] = dict[node] / len(parent)
    ...
    for node, distance in node2distances:
      if node not in list:
        ...
        for parent in node2parents[node]:
          dict[parent] = dict[node] / len(node2parents[node])
          r = tuple(sorted((node, parent))) 
          result[r] = dict[node] / len(node2parents[node])
...
def approximate_betweenness(graph, max_depth):
    ...
    appro_bw = defaultdict(int)
    for node in graph.nodes():
      node2distances, node2num_paths, node2parents = bfs(graph, node, max_depth)
      bottomup = bottom_up(node, node2distances, node2num_paths, node2parents)

      for key, value in bottomup.items(): 
        appro_bw[key] += value/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    G = graph.copy()
    edges = approximate_betweenness(G, max_depth)
    edge_list = sorted(edges.items(), key=lambda x: x[:][1], reverse=True)
    components = [c for c in nx.connected_component_subgraphs(G)]

    i = 0
    while len(components) == 1:
      G.remove_edge(*edge_list[i][0])
...
def cut(S, T, graph):
    ...
    for s in S:
      for t in T:
        st = (s, t)
        ts = (t, s)
        if (graph.has_edge(*st) or graph.has_edge(*ts)):
          cut_set = cut_set + 1
...
def norm_cut(S, T, graph):
    ...
    cut_score = cut(S,T,graph)
    Svolume = volume(S, graph)
    Tvolume = volume(T, graph)

    return float(cut_score/Svolume + cut_score/Tvolume)
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
      ...
      nc = norm_cut(components[0], components[1], graph)
      score_max.append((i,nc))
...
def make_training_graph(graph, test_node, n):
    ...
    for i in range(n):
      G.remove_edge(*(test_node, neighbours[i]))
...
def jaccard(graph, node, k):
    ...
    for n in sorted(graph.nodes()):
      if node!=n and not graph.has_edge(node, n):
        ...
        scores.append(((node,n), 1. * len(neighbours & neighbours2) / len(neighbours | neighbours2)))
...
def path_score(graph, root, k, beta):
    ...
    for node in graph.node:
      if (root,node) not in edges and (node, root) not in edges and root != node:
        path_scores.append(((root, node), (beta ** node2distances[node]) * node2num_paths[node]))

    return sorted(path_scores, key=lambda x: x[1], reverse=True)[:k]  
